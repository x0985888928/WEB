    ...
    <div class="tabBox">
				...
				<?php
					if(isset($_POST["per"])){
				  	...
				  	if($filename4!=NULL){
						...
						mysqli_query($link, "UPDATE `表演者` SET `藝名` = '$na'  WHERE `帳號` = '$ac' ") or die("qd5ueary");
						$type1 = $_POST['type1'];
						if($type1==1)
      ...
      <div id="tab3" class="view">
        ...
        <div class="orderBox">
          <div id="finish" class="order">
            <?php
			        ...
			        if(isset($_SESSION['account'])){
								...
								mysqli_select_db($link, 'traum') or die("db");
								mysqli_query($link,"SET CHARACTER SET utf8");
								$sql = "SELECT * FROM `訂單` WHERE (`表演者`='$ac' OR`聯絡人`= '$ac' ) and `完成與否`= 1 ";
							...
							/*		$per = 10;
									...
									$sql7 = "SELECT * FROM `訂單`  WHERE `表演者` = '$ac' and `完成與否`=1 ";

									$result = mysqli_query($link,$sql7) or die("asd");
									echo "表演
									<table>
									<tr>
										<td>訂單編號</td>
									...
									echo "</table></form>";
									$sql3 = "SELECT * FROM `訂單`  WHERE `聯絡人` = '$ac' and `完成與否`=1 ";
				        ...
				        if(isset($_SESSION['account']))
								{
									...
									$sql = "SELECT * FROM `訂單` WHERE (`表演者`='$ac' OR `聯絡人`= '$ac' ) and `完成與否`= 0 ";
									$resultf = mysqli_query($link, $sql) or die("qdueary");
									$data_nums = mysqli_num_rows($resultf);
							...
							//			$start = ($page-1)*$per;
							  		$sql2 = "SELECT * FROM `訂單`  WHERE `表演者` = '$ac' and `完成與否`= 0 ";
										$result = mysqli_query($link,$sql2) or die("asd");
										...
										echo "</table></form>";
										$sql3 = "SELECT * FROM `訂單`  WHERE `聯絡人` = '$ac' and `完成與否`=0 ";
											...
											if($row = mysqli_fetch_assoc($result)){
												if($row["送方"]==$ac){
													...
													echo" ".">$row[日期]</p>  </div></div>";
														...
														{
															$in = $_POST['sssss'];
															$sd="INSERT INTO `聊天室`(`收方`, `送方`, `內容`, `日期`, `已讀與否`, `編號`)
																VALUES('$row2222[帳號]','$ac','$in',CURRENT_TIMESTAMP,'0','0')";
                         mysqli_query($link, $sd) or die("quseary");
														}
				  ?>
          ...
          <h4><?php echo"$row2222[帳號]";?></h4>
          <div class="feedback">
            <img src="<?php echo"$row2222[圖片]";?>" alt="">
			...
			$link = mysqli_connect('localhost', 'project', '0931821282','traum');
							mysqli_select_db($link, 'traum') or die("db");
							mysqli_query($link,"SET CHARACTER SET utf8");
							$sql = "SELECT * FROM `聊天室` WHERE (`收方`='$ac' OR`送方`= '$ac' ) and (`收方`='$row2222[帳號]' OR`送方`='$row2222[帳號]')  ORDER BY `日期` ASC";
							$result = mysqli_query($link, $sql) or die("queary");
							while($row = mysqli_fetch_assoc($result))
							{
								if($row['收方']==$ac)
								...
								{
								  echo "<div class="."conversation"."><div class="."conversation".">
					          <p>$row[內容]</p>
					          </div></div>";
							  }
							}
			 ...
			 <form action="" method="post" name ="form1">
				<div id="texting">
					<input type="text" name="sssss" value="">
					<button type="button"  onClick="sss.click()">Enter
				</button>
				<input name ="sss" type="submit" style="display: none;">
			</form>
